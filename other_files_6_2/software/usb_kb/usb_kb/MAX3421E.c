...
void MAXreg_wr(BYTE reg, BYTE val) {
	BYTE arr[2] = {reg + 2, val};
	int return_code = alt_avalon_spi_command(SPI_0_BASE,0,2,arr,0,NULL,0);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int return_code = alt_avalon_spi_command(SPI_0_BASE,0,nbytes+1,arr,0,NULL,0);
	...
	return (data+nbytes);
//	reg = reg + 2;
//	int x = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, 0, NULL, ALT_AVALON_SPI_COMMAND_MERGE);
//	x = alt_avalon_spi_command(SPI_0_BASE, 0, nbytes, data, 0, NULL, 0);
//	if(x<0){
//		printf("error");
//	}
//	return (data + nbytes);
...
BYTE MAXreg_rd(BYTE reg) {
	BYTE val = 0;
	int x = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, 1, &val, 0);
	if(x<0){
		printf("error");
	}
	return val;
